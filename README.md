...
| 记忆管理 | 自动记忆、手动记忆、搜索、删除、按会话清空 | `backend/app/api/v1/memory.py` |
...
1. 用户消息中出现“记住、以后、偏好、默认、称呼”等触发词时，系统自动保存记忆。
2. 聊天页“记忆管理”支持手动新增、搜索、删除和清空。
3. 每次问答前检索当前会话相关记忆，并注入 Prompt。
4. Prompt 明确要求记忆只作为用户偏好和上下文线索，不能作为政策依据。
